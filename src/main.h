...
static const int LAST_POW_BLOCK = 5000113;
...
static const int64_t COIN_YEAR_REWARD = 260* CENT;
...
static const uint256 hashGenesisBlockTestNet("0x38624e3834cfdc4410a5acbc32f750171aadad9620e6ba6d5c73201c16f7c8d1");
...
public:
    ...
    */
    ...
    {
        // Netcoin: 1440 blocks found a day. Priority cutoff is 100 netcoin day / 250 bytes.
        return dPriority > COIN * 1440 / 250; // NetCoin: 1440 blocks found a day. Priority cutoff is 1 netcoin day / 250 bytes.
    }

    int64_t GetMinFee(unsigned int nBlockSize=1, enum GetMinFee_mode mode=GMF_BLOCK, unsigned int nBytes = 0, bool fAllowFree = false) const;
    ...
    std::string ToString() const
    {
        ...
        str += strprintf("(hash=%s, nTime=no, ver=%d, vin.size=%"PRIszu", vout.size=%"PRIszu", nLockTime=%d)\n",
    ...
    bool GetCoinAge(CTxDB& txdb, unsigned int nTxTime, uint64_t& nCoinAge) const;  // ppcoin: get transaction coin age
...
public:
    ...
    (
        ...
        if (!(nType & (SER_GETHASH|SER_BLOCKHEADERONLY)))
        {
            ...
            // BlockSig only in block version 2
