...
unsigned int nTargetSpacing = 60;								// NetCoin: 60 sec
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;
unsigned int nStakeMinAge = 3 * 60 * 60; // 8 hours
unsigned int nStakeMaxAge = 604800; // 7 days
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (fTestNet) {
            ...
            if (pindexLast->nHeight+1 >= 218500) { DiffMode = 2; }
    }
...
bool CBlock::AcceptBlock()
{
    ...
    // HARD FORK: switch to version 2 starting from PoS
    if (nHeight > LAST_POW_BLOCK && nVersion < 2)
        return DoS(100, error("AcceptBlock() : reject version <3 block at height %d", nHeight));
    if (nHeight <= LAST_POW_BLOCK && nVersion > 1)
        return DoS(100, error("AcceptBlock() : reject version >1 block at height %d", nHeight));
    ...
    // Unfortunately there are bad version 2 blocks in Netcoin blockchain (e.g. block 6)
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nNonce   = !fTestNet ? 12344321 : 12344321;
